...
This is a sample of the data set. The column **My.Review** is left out, due to the fact that it made the row height too large.
```{r echo=FALSE}
#create a table
...
So we can assume that Viet titles are encoded with UTF-8, since they have some non-ASCII characters. Fortunately, we had a small dataset so I was able to check if I have left out any books. This might not hold true if a book title has no non-ASCII character and we have a big data set. 
...
We will find a way to make the data reflect the true pages read per month later in the analysis.
...
#### 2.5.b.Reviews
Does the length of reviews correspond to rating? Let's find out.
First, let's get a dataframe for **My.Review**'s length and **My.Rating**.
```{r}
# dataframe for review length and my.rating
ratereview <- tibble(rvlength = stri_length(rdata$My.Review),rating = rdata$My.Rating,title=rdata$Title)
ratereview
ggplot(ratereview,aes(x=as.factor(rating),y=rvlength))+geom_point()
ratereview$title[which.max(ratereview$rvlength)]
```
...
I could also learn more by finding and reading other analysis on Goodreads data.  

This is one of my first projects I did on something I like. And I was surprised with the amount of time and obsession I put in it. It felt great. I will recreate this analysis every year. 
